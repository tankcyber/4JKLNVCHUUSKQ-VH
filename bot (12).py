...
def format_response_for_telegram(text: str) -> str:
    ...
    # Чистим LaTeX-разметку для Telegram
    text = re.sub(r'\\\[', '', text)  # убираем \[
    text = re.sub(r'\\\]', '', text)  # убираем \]
    text = re.sub(r'\\boxed\{([^}]*)\}', r'\1', text)  # \boxed{...} → ...
    text = re.sub(r'\\displaystyle', '', text)  # \displaystyle
    text = re.sub(r'\\text\{([^}]*)\}', r'\1', text)  # \text{...} → ...
    # \frac{a}{b} → a/b  (упрощённо, не вложенные)
...
async def clear_memory_button(message: Message):
    ...
    if await require_subscription(user_id, message):
        return
...
async def require_subscription(user_id: int, message: Message) -> bool:
    """Проверить подписку. Если не подписан — отправить предупреждение и вернуть True."""
    is_subscribed = await check_user_subscription(user_id)
    if not is_subscribed:
        await message.answer(
...
async def process_token_purchase(callback: CallbackQuery):
    ...
    text = (
        ...
        f"📌 Кошелек ЮMoney: `{YOOMONEY_WALLET}`\n"
        f"💵 Сумма: {package['price']}₽\n\n"
        f"📝 **В комментарии к переводу укажите:**\n"
        f"`{user_id}`\n\n"
        f"После оплаты нажмите **«✅ Я оплатил»** — админ проверит и зачислит токены."
    )
    
    builder = InlineKeyboardBuilder()
    if payment_url:
        builder.button(text="💳 Перейти к оплате", url=payment_url)
    builder.button(text="✅ Я оплатил", callback_data=f"paid_{package_key}")
...
async def i_paid_callback(callback: CallbackQuery):
    """Пользователь нажал «Я оплатил» — уведомляем админов"""
    user_id = callback.from_user.id
    package_key = callback.data.replace("paid_", "")
    package = TOKEN_PACKAGES.get(package_key)
    if not package:
        await callback.answer("❌ Пакет не найден")
        return
    ...
    for admin_id in ADMIN_IDS:
        try:
            text = (
                f"🔔 **Запрос на зачисление токенов!**\n\n"
            ...
            builder = InlineKeyboardBuilder()
            builder.button(text="✅ Выдать токены", callback_data=f"approve_{user_id}_{package_key}")
            builder.button(text="❌ Отказать", callback_data=f"reject_{user_id}_{package_key}")
            builder.adjust(2)
    ...
    await callback.message.edit_text(
        ...
        f"Админ проверит оплату и зачислит токены в ближайшее время.\n"
        f"Если прошло много времени — напишите @SedoyDiada",
        reply_markup=None
    )
...
async def approve_payment(callback: CallbackQuery):
    """Админ подтвердил оплату — зачисляем токены"""
    ...
    _, target_id_str, package_key = parts
    ...
    add_admin_tokens(target_id, package["tokens"])
    ...
    try:
        await bot.send_message(
            ...
            f"💎 Зачислено: {package['tokens']:,} токенов\n"
            f"📦 Пакет: {package['label']}\n\n"
    ...
    await callback.message.edit_text(
        ...
        f"➖➖➖➖➖➖➖➖➖➖\n"
        f"✅ **Токены выданы!**\n"
        f"💎 +{package['tokens']:,} токенов пользователю {target_id}",
        parse_mode="Markdown",
        reply_markup=None
    )
    await callback.answer(f"✅ {package['tokens']:,} токенов выдано пользователю {target_id}", show_alert=True)
...
async def reject_payment(callback: CallbackQuery):
    ...
    try:
        await bot.send_message(
            ...
            f"Админ не обнаружил ваш перевод.\n"
            f"Проверьте правильность перевода и попробуйте снова.\n"
            f"По вопросам: @SedoyDiada"
        )
    except Exception as e:
        logging.error(f"Не удалось уведомить пользователя {target_id}: {e}")
...
async def main():
    ...
    # Запускаем периодическую проверку сброса балансов
    ...
    print(f"📅 Дневной лимит: {DAILY_LIMIT} токенов")
    if YOOMONEY_WALLET:
        print(f"💳 ЮMoney: настроен (ручная выдача админом)")
    else:
        print(f"💳 ЮMoney: НЕ настроен")
    print("=" * 50)
