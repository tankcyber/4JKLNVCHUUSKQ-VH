...
# ================= НАСТРОЙКИ СТОИМОСТИ =================
# Теперь токены пользователя = реальным токенам API 1:1
# Множители моделей и скорости применяются к реальным токенам
MODELS = {
    "gpt-5.4": {
        "name": "GPT-5.4", 
        "multiplier": 1.0, 
    ...
    "gpt-5.5": {
        ...
        "multiplier": 4.5, 
        "api_name": "gpt-5.5",
        "description": "Мощная модель (x4.5)"
    },
    "gpt-image-2": {
        "name": "GPT-IMAGE 2", 
        "multiplier": 3.0, 
        "api_name": "gpt-image-2",
        "description": "Для изображений (x3)"
    }
}
...
async def calculate_cost(user_id: int, prompt_tokens: int, completion_tokens: int = 0):
    ...
    logging.info(f"💰 Расчет стоимости: реальные_токены_API={total_real_tokens}, "
                f"model_mult={model_mult}, priority_mult={priority_mult}, "
                f"списано_токенов_пользователя={user_tokens:.2f}")
...
async def calculate_image_cost(user_id: int, is_edit: bool = False) -> float:
    ...
    # Токены пользователя = реальные токены * множители
    user_tokens = real_tokens_for_image * model_mult * priority_mult
...
def get_models_inline_keyboard():
    ...
    for key, val in MODELS.items():
        builder.button(text=f"{val['name']} (x{val['multiplier']}) — {val['description']}", 
...
async def generate_image(user_id: int, prompt: str, thinking_message: Message = None):
    ...
    if total_balance < cost:
        ...
        return None, None, f"❌ Недостаточно токенов! Нужно: {cost:.0f}, есть: {total_balance:.0f}"
        ...
        try:
            async with session.post(
                ...
                caption = (
                    ...
                    f"💎 Дневной баланс: {balance_info['daily']:.0f} | Админские: {balance_info['admin']:.0f} | Всего: {balance_info['total']:.0f}\n"
                    f"📊 Списано: {cost:.0f} токенов"
                )
        ...
        try:
            async with session.post(API_URL, headers=HEADERS, json=payload, timeout=60) as resp:
                ...
                if 'usage' in data:
                    ...
                    logging.info(f"✅ API usage: prompt={prompt_tokens}, completion={completion_tokens}, total={total_real_tokens} реальных токенов API")
                else:
                ...
                logging.info(f"💰 Стоимость запроса: {user_tokens_cost:.2f} токенов пользователя (реальных токенов API: {total_real_tokens})")
                ...
                balance_info = get_balance_info(user_id)
                model_mult = MODELS[model_key]['multiplier']
                priority_mult = PRIORITY_MODES[priority_mode]['multiplier']
                
                footer = (f"\n\n---\n"
                         f"💎 Дневной: {balance_info['daily']:.0f} | Админские: {balance_info['admin']:.0f} | Всего: {balance_info['total']:.0f}\n"
                         f"🧠 {MODELS[model_key]['name']} | ⚡ {PRIORITY_MODES[priority_mode]['emoji']}\n"
                         f"📊 Реальных токенов API: {total_real_tokens}\n"
                         f"💰 Списано: {user_tokens_cost:.0f} (x{model_mult} * x{priority_mult})")
...
async def models_button(message: Message):
    ...
    text = (
        "🎛 Выберите модель ИИ\n\n"
        "📊 Доступные модели и их множители:\n"
        "• GPT-5.4 — базовый тариф (x1.0)\n"
        "• GPT-5.4 Mini — экономная (x0.9) - СКИДКА 10%\n"
        "• GPT-5.5 — мощная (x4.5)\n"
        "• GPT-IMAGE 2 — генерация и редактирование (x3.0)\n\n"
        "🖼 GPT-IMAGE 2:\n"
        "• Текст → сгенерирует картинку\n"
        "• Фото + подпись → отредактирует картинку\n\n"
        "💡 Формула списания:\n"
        "реальные_токены_API × множитель_модели × множитель_скорости"
    )
...
async def priority_button(message: Message):
    ...
    text = (
        ...
        "• 🐢 Обычный режим (x1.0) — стандартная скорость\n"
        "• ⚡ Fast Speed (x2.0) — приоритетные запросы, в 2 раза дороже\n\n"
...
async def profile_button(message: Message):
    ...
    text = (
        ...
        f"   └ Множитель: x{model_info['multiplier']}\n\n"
        f"⚡ Скорость: {priority_info['emoji']} {priority_info['name']}\n"
        f"   └ Множитель: x{priority_info['multiplier']}\n\n"
...
async def buy_tokens_button(message: Message):
    ...
    text = (
        ...
        "📊 Цены (1 токен = 1 реальный токен API):\n"
        "• 100 000 токенов — 20₽\n"
        "• 500 000 токенов — 90₽\n"
...
async def set_user_model(callback: CallbackQuery):
    ...
    if model_key in MODELS:
        ...
        text = (
            ...
            f"📊 Множитель: x{model_info['multiplier']}\n"
            f"💰 Теперь токены будут списываться: реальные_токены_API × {model_info['multiplier']} × множитель_скорости"
        )
...
async def set_priority(callback: CallbackQuery):
    ...
    if priority_key in PRIORITY_MODES:
        ...
        text = (
            ...
            f"📊 Множитель: x{priority_info['multiplier']}\n"
            f"💰 Теперь токены будут списываться: реальные_токены_API × множитель_модели × {priority_info['multiplier']}"
        )
        await safe_edit_message(callback, text)
    await callback.answer()
...
async def main():
    ...
    print(f"💰 Токены пользователя = реальные токены API × множители")
