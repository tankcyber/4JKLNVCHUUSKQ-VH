...
def find_user_by_username(username):
    ...
    res = cur.execute('SELECT user_id FROM users WHERE username = ?', (clean_username,)).fetchone()
    conn.close()
    return res
...
async def call_codex_api(user_id, prompt, thinking_message: Message = None, image_url=None):
    ...
    # Если есть картинка (для моделей с vision)
    if image_url and model_key in ["gpt-image-2", "gpt-5.5"]:
...
async def show_models(callback: CallbackQuery):
    text = (
        ...
        "• GPT-5.5 — мощная (x4.5)\n"
        "• GPT-IMAGE 2 — для изображений (x3)\n\n"
...
# Убедитесь, что класс AdminStates определён правильно:
class AdminStates(StatesGroup):
    waiting_for_user_id = State()
    waiting_for_token_amount = State()

@dp.callback_query(F.data.in_(["admin_ban", "admin_give", "admin_take"]))
async def admin_action_handler(callback: CallbackQuery, state: FSMContext):
    """Обработчик для выдачи/забирания токенов и бана"""
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("⛔ Нет доступа", show_alert=True)
        return
    
    # Определяем действие
    if callback.data == "admin_give":
        action_type = "give"
        action_name = "выдать токены"
    elif callback.data == "admin_take":
        action_type = "take"
        action_name = "забрать токены"
    elif callback.data == "admin_ban":
        action_type = "ban"
        action_name = "забанить/разбанить"
    else:
        return
    
    # Сохраняем действие в состояние
    await state.update_data(admin_action=action_type)
    ...
    await callback.message.answer(
        f"🔍 Введите Telegram ID или username (без @) пользователя, чтобы {action_name}:"
    )
