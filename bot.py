...
# ================= НАСТРОЙКИ СТОИМОСТИ =================
# Базовая стоимость 1 реального токена API в токенах пользователя
# Например: 100 токенов пользователя за 1K реальных токенов = 0.1 токена пользователя за 1 реальный токен
BASE_PRICE_PER_1K = 100  # 100 токенов пользователя за 1000 реальных токенов API
...
async def calculate_cost(user_id: int, prompt_tokens: int, completion_tokens: int = 0):
    """
    Рассчитать стоимость в токенах пользователя на основе реальных токенов от API
    
    Формула: (реальные_токены / 1000) * BASE_PRICE_PER_1K * model_mult * priority_mult
    """
    ...
    # Стоимость в токенах пользователя
    cost = (total_real_tokens / 1000) * (BASE_PRICE_PER_1K * model_mult * priority_mult)
    
    logging.info(f"💰 Расчет стоимости: реальные_токены={total_real_tokens}, "
                f"model_mult={model_mult}, priority_mult={priority_mult}, "
                f"cost={cost:.2f} токенов пользователя")
    
    return cost

async def calculate_image_cost(user_id: int, is_edit: bool = False) -> float:
    """
    Стоимость генерации/редактирования изображения в токенах пользователя
    """
    user = get_user(user_id)
    if not user:
        return 0
    ...
    # Реальные токены, которые тратит API на генерацию изображения
    real_tokens_for_image = 5000 if is_edit else 3000
    
    # Стоимость в токенах пользователя
    cost = (real_tokens_for_image / 1000) * (BASE_PRICE_PER_1K * model_mult * priority_mult)
    
    logging.info(f"🖼 Стоимость изображения: is_edit={is_edit}, "
                f"real_tokens={real_tokens_for_image}, cost={cost:.2f} токенов пользователя")
...
def get_models_inline_keyboard():
    ...
    for key, val in MODELS.items():
        price_per_1k = int(BASE_PRICE_PER_1K * val['multiplier'])
        builder.button(text=f"{val['name']} — {price_per_1k} ток/1K токенов API ({val['multiplier']}x)", 
        ...
        try:
            async with session.post(
                ...
                caption = (
                    ...
                    f"💎 Остаток: {balance:.0f} токенов | 🖼 {MODELS['gpt-image-2']['name']} | 📊 3000 токенов API"
                )
        ...
        try:
            async with session.post(
                ...
                caption = (
                    ...
                    f"💎 Остаток: {balance:.0f} токенов | 🖼 {MODELS['gpt-image-2']['name']} | 📊 5000 токенов API"
                )
        ...
        try:
            async with session.post(API_URL, headers=HEADERS, json=payload, timeout=60) as resp:
                ...
                if 'usage' in data:
                    ...
                    total_real_tokens = data['usage'].get('total_tokens', 0)
                    
                    if total_real_tokens == 0 and prompt_tokens > 0:
                        total_real_tokens = prompt_tokens + completion_tokens
                    
                    logging.info(f"✅ API usage: prompt={prompt_tokens}, completion={completion_tokens}, total={total_real_tokens}")
                else:
                ...
                # Расчёт стоимости на основе реальных токенов от API
                total_cost = await calculate_cost(user_id, prompt_tokens, completion_tokens)
                
                logging.info(f"💰 Стоимость запроса: {total_cost:.2f} токенов пользователя (реальных токенов API: {total_real_tokens})")
                ...
                # Формируем компактный футер с информацией о реальных токенах
                footer = f"\n\n---\n💎 Остаток: {user_final[2]:.0f} токенов | 🧠 {MODELS[model_key]['name']} | 📊 {total_real_tokens} токенов API"
...
async def cmd_start(message: Message):
    ...
    price_per_1k = int(BASE_PRICE_PER_1K * model_info['multiplier'])
    
    await message.answer(
        f"✨ Привет, {message.from_user.first_name}!\n\n"
        f"🤖 Я работаю через API codex.sale\n\n"
        f"💰 Баланс: {user[2]} токенов\n"
        f"🧠 Модель: {model_info['name']} ({model_info['multiplier']}x) — {price_per_1k} ток/1K токенов API\n"
...
async def models_button(message: Message):
    ...
    text = (
        ...
        "💡 Цена указана за 1K реальных токенов API"
    )
...
async def profile_button(message: Message):
    ...
    text = (
        ...
        f"   └ Стоимость: {price_per_1k} ток/1K токенов API\n\n"
...
async def set_user_model(callback: CallbackQuery):
    ...
    if model_key in MODELS:
        ...
        text = (
            ...
            f"📊 Коэффициент: {model_info['multiplier']}x\n"
            f"💰 Стоимость: {price_per_1k} токенов за 1K реальных токенов API\n"
...
async def main():
    ...
    print(f"💰 Базовая цена: {BASE_PRICE_PER_1K} токенов за 1K реальных токенов API")
    print(f"🤖 Доступные модели:")
    for key, val in MODELS.items():
        price = int(BASE_PRICE_PER_1K * val['multiplier'])
        print(f"   - {val['name']} ({val['api_name']}) | x{val['multiplier']} | {price} ток/1K токенов API")
